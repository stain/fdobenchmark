 ...
 * - Merged from: 
 *   https://github.com/spring-projects/spring-hateoas/blob/2.4.1/src/main/java/org/springframework/hateoas/LinkRelation.java
 *   https://github.com/spring-projects/spring-hateoas/blob/2.4.1/src/main/java/org/springframework/hateoas/IanaLinkRelations.java
 *   https://github.com/spring-projects/spring-hateoas/blob/2.4.1/src/main/java/org/springframework/hateoas/StringLinkRelation.java
...
public interface LinkRelation {
	...
	/**
	 * Returns whether the given {@link LinkRelation} is logically the same as the
	 * current one, independent of implementation, i.e. whether the plain
	 * {@link String} values match.
	 *
	...
	/**
	 * Returns a new {@link LinkRelation} with its relation mapped by the given
	 * function, unless it is an IANA one. Implementors are encouraged to override
	 * this method to redeclare the return type to be itself.
	 *
	...
	public final class IanaLinkRelations {
		...
		/**
		 * Identifies the entity that blocks access to a resource following receipt of a
		 * legal demand.
		 *
		...
		/**
		 * Indicates that the link target is preferred over the link context for the
		...
		/**
		 * The document linked to was later converted to the document that contains this
		 * link relation. For example, an RFC can have a link to the Internet-Draft that
		 * became the RFC; in that case, the link relation would be "convertedFrom".
		 *
		 ...
		 * representation of either A or B, neither are there any further constraints on
		 * either resource.
		 *
		 ...
		 * the link context, and that the user agent ought to resolve as early as
		 * possible.
		 *
		...
		/**
		 * refers to a resource associated with a time interval that ends before the
		 * beginning of the time interval associated with the context resource
		 *
		 * @see {@link https://www.w3.org/TR/owl-time/#time:intervalAfter section
		 *      4.2.21}
		 */
		...
		/**
		 * refers to a resource associated with a time interval that begins after the
		 * end of the time interval associated with the context resource
		 *
		 * @see {@link https://www.w3.org/TR/owl-time/#time:intervalBefore section
		 *      4.2.22}
		 */
		...
		/**
		 * refers to a resource associated with a time interval that begins after the
		 * beginning of the time interval associated with the context resource, and ends
		 * before the end of the time interval associated with the context resource
		 *
		 * @see {@link https://www.w3.org/TR/owl-time/#time:intervalContains section
		 *      4.2.23}
		 */
		...
		/**
		 * refers to a resource associated with a time interval that begins after the
		 * end of the time interval associated with the context resource, or ends before
		 * the beginning of the time interval associated with the context resource
		 *
		 * @see {@link https://www.w3.org/TR/owl-time/#time:intervalDisjoint section
		 *      4.2.24}
		 */
		...
		/**
		 * refers to a resource associated with a time interval that begins before the
		 * beginning of the time interval associated with the context resource, and ends
		 * after the end of the time interval associated with the context resource
		 *
		 * @see {@link https://www.w3.org/TR/owl-time/#time:intervalDuring section
		 *      4.2.25}
		 */
		 ...
		 * coincides with the beginning of the time interval associated with the context
		 * resource, and whose end coincides with the end of the time interval
		 * associated with the context resource
		 *
		 * @see {@link https://www.w3.org/TR/owl-time/#time:intervalEquals section
		 *      4.2.26}
		 */
		...
		/**
		 * refers to a resource associated with a time interval that begins after the
		 * beginning of the time interval associated with the context resource, and
		 * whose end coincides with the end of the time interval associated with the
		 * context resource
		 *
		 * @see {@link https://www.w3.org/TR/owl-time/#time:intervalFinishedBy section
		 *      4.2.27}
		 */
		 ...
		 * context resource
		 *
		 * @see {@link https://www.w3.org/TR/owl-time/#time:intervalFinishes section
		 *      4.2.28}
		 */
		...
		/**
		 * refers to a resource associated with a time interval that begins before or is
		 * coincident with the beginning of the time interval associated with the
		 * context resource, and ends after or is coincident with the end of the time
		 ...
		 * resource
		 *
		 * @see {@link https://www.w3.org/TR/owl-time/#time:intervalMeets section
		 *      4.2.30}
		 */
		 ...
		 * resource
		 *
		 * @see {@link https://www.w3.org/TR/owl-time/#time:intervalMetBy section
		 *      4.2.31}
		 */
		 ...
		 * refers to a resource associated with a time interval that begins before the
		 * beginning of the time interval associated with the context resource, and ends
		 * after the beginning of the time interval associated with the context resource
		 *
		 * @see {@link https://www.w3.org/TR/owl-time/#time:intervalOverlappedBy section
		 *      4.2.32}
		 */
		...
		/**
		 * refers to a resource associated with a time interval that begins before the
		 * end of the time interval associated with the context resource, and ends after
		 * the end of the time interval associated with the context resource
		 *
		 * @see {@link https://www.w3.org/TR/owl-time/#time:intervalOverlaps section
		 *      4.2.33}
		 */
		 ...
		 * refers to a resource associated with a time interval whose beginning
		 * coincides with the beginning of the time interval associated with the context
		 * resource, and ends before the end of the time interval associated with the
		 * context resource
		 *
		 * @see {@link https://www.w3.org/TR/owl-time/#time:intervalStartedBy section
		 *      4.2.34}
		 */
		 ...
		 * refers to a resource associated with a time interval whose beginning
		 * coincides with the beginning of the time interval associated with the context
		 * resource, and ends after the end of the time interval associated with the
		 * context resource
		 *
		 * @see {@link https://www.w3.org/TR/owl-time/#time:intervalStarts section
		 *      4.2.35}
		 */
		 ...
		 * ("Link-based Resource Descriptor Document") resource. See RFC6415 for
		 * information about processing this relation type in host-meta documents. When
		 * used elsewhere, it refers to additional links and other metadata. Multiple
		 * instances indicate additional LRDD resources. LRDD resources MUST have an
		...
		/**
		 * Indicates that the link's context is a part of a series, and that the next in
		...
		/**
		 * Indicates that the context’s original author or publisher does not endorse
		 * the link target.
		 *
		...
		/**
		 * Used to indicate an origin that will be used to fetch required resources for
		 * the link context. Initiating an early connection, which includes the DNS
		 * lookup, TCP handshake, and optional TLS negotiation, allows the user agent to
		 ...
		 * required by the next navigation from the link context, and that the user
		 * agent ought to fetch, such that the user agent can deliver a faster response
		...
		/**
		 * Used to identify a resource that might be required by the next navigation
		 * from the link context, and that the user agent ought to fetch and execute,
		 * such that the user agent can deliver a faster response once the resource is
		...
		/**
		 * Identifies the root of RESTCONF API as configured on this HTTP server. The
		 * "restconf" relation defines the root of the API defined in RFC8040.
		...
		/**
		 * Identifies a target URI that supports the Webmention protcol. This allows
		 * clients that mention a resource in some form of publishing process to contact
		 * that endpoint and inform it that this resource has been mentioned.
		 *
		...
		static {
			...
			LINK_RELATIONS = new HashSet<LinkRelation>(Arrays.asList(ABOUT, ALTERNATE, APPENDIX, ARCHIVES, AUTHOR, BLOCKED_BY,
					BOOKMARK, CANONICAL, CHAPTER, CITE_AS, COLLECTION, CONTENTS, CONVERTED_FROM, COPYRIGHT, CREATE_FORM,
					CURRENT, DESCRIBED_BY, DESCRIBES, DISCLOSURE, DNS_PREFETCH, DUPLICATE, EDIT, EDIT_FORM, EDIT_MEDIA,
					ENCLOSURE, FIRST, GLOSSARY, HELP, HOSTS, HUB, ICON, INDEX, INTERVAL_AFTER, INTERVAL_BEFORE,
					INTERVAL_CONTAINS, INTERVAL_DISJOINT, INTERVAL_DURING, INTERVAL_EQUALS, INTERVAL_FINISHED_BY,
					INTERVAL_FINISHES, INTERVAL_IN, INTERVAL_MEETS, INTERVAL_MET_BY, INTERVAL_OVERLAPPED_BY,
					INTERVAL_OVERLAPS, INTERVAL_STARTED_BY, INTERVAL_STARTS, ITEM, LAST, LATEST_VERSION, LICENSE, LRDD,
					MEMENTO, MONITOR, MONITOR_GROUP, NEXT, NEXT_ARCHIVE, NOFOLLOW, NOREFERRER, ORIGINAL, PAYMENT,
					PINGBACK, PRECONNECT, PREDECESSOR_VERSION, PREFETCH, PRELOAD, PRERENDER, PREV, PREV_ARCHIVE,
					PREVIEW, PREVIOUS, PRIVACY_POLICY, PROFILE, RELATED, REPLIES, RESTCONF, SEARCH, SECTION, SELF,
					SERVICE, START, STYLESHEET, SUBSECTION, SUCCESSOR_VERSION, TAG, TERMS_OF_SERVICE, TIMEGATE, TIMEMAP,
					TYPE, UP, VERSION_HISTORY, VIA, WEBMENTION, WORKING_COPY, WORKING_COPY_OF));
		}
		...
		/**
		 * Is this relation an IANA standard? Per RFC 8288, parsing of link relations is
		 * case insensitive.
		 *
		 * @param relation must not be {@literal null}.
		 * @return boolean
